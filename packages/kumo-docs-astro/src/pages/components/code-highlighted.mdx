...
<ComponentSection>
  ...
  <CodeBlock code={`import { ShikiProvider, CodeHighlighted } from "@cloudflare/kumo/code";`} lang="tsx" />
...
<p>
  Emphasize specific lines with
  `highlightLines`
  (1-indexed).
</p>
<ComponentExample
  code={`<CodeHighlighted code={code} lang="typescript" highlightLines={[5, 6]} />`}
>
...
<p>
  Customize the highlight color with the
...
<ComponentExample
  code={`<CodeHighlighted code={code} lang="typescript" showLineNumbers />`}
>
...
<p>
  Add a copy-to-clipboard button with
  `showCopyButton`
  .
</p>
<ComponentExample
  code={`<CodeHighlighted code={code} lang="bash" showCopyButton />`}
>
